    ...
    for(;;)
    {
        ...
        if(readCapSenseFlag == 1)
        {
            if(!CapSense_IsBusy())
            {
                ...
                {
                    i2cReadBuffer[i+13] = CapSense_dsRam.snsList.row3[i].raw[0];
                    i2cReadBuffer[i+16] = CapSense_dsRam.snsList.row4[i].raw[0];
                    i2cReadBuffer[i+19] = CapSense_dsRam.snsList.row5[i].raw[0];
                    i2cReadBuffer[i+22] = CapSense_dsRam.snsList.row6[i].raw[0];
                    i2cReadBuffer[i+25] = CapSense_dsRam.snsList.row7[i].raw[0];
                }
                for(unsigned int i=0; i<1; ++i) // Proxi
                {
                    i2cReadBuffer[i+28] = CapSense_dsRam.snsList.proxi[i].raw[0]; 
                }
