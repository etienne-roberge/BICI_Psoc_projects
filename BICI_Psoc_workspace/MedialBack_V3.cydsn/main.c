    ...
    for(;;)
    {
        ...
        if(readCapSenseFlag == 1)
        {
            if(!CapSense_IsBusy())
            {
                ...
                for(unsigned int i=0; i<4; ++i) // Rows0-1
                {
                    i2cReadBuffer[i+1] = CapSense_dsRam.snsList.row0[i].raw[0]; 
                    i2cReadBuffer[i+5] = CapSense_dsRam.snsList.row1[i].raw[0];
                }
                for(unsigned int i=0; i<6; ++i) // Row2
                {
                    i2cReadBuffer[i+9] = CapSense_dsRam.snsList.row2[i].raw[0];
                }
                for(unsigned int i=0; i<3; ++i) // Row3-4
                {
                    i2cReadBuffer[i+15] = CapSense_dsRam.snsList.row3[i].raw[0];
                    i2cReadBuffer[i+18] = CapSense_dsRam.snsList.row4[i].raw[0];
                }
                for(unsigned int i=0; i<5; ++i) // Row5
                {
                    i2cReadBuffer[i+21] = CapSense_dsRam.snsList.row5[i].raw[0];
                }
                for(unsigned int i=0; i<4; ++i) // Row6
                {
                    i2cReadBuffer[i+26] = CapSense_dsRam.snsList.row6[i].raw[0];
                }
                for(unsigned int i=0; i<1; ++i) // Row7
                {
                    i2cReadBuffer[i+30] = CapSense_dsRam.snsList.row7[i].raw[0];
                }
                for(unsigned int i=0; i<1; ++i) // Proxi
                {
                    i2cReadBuffer[i+31] = CapSense_dsRam.snsList.proxi[i].raw[0]; 
                }
